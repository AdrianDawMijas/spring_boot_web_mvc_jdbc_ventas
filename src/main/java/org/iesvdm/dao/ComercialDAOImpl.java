...
public class ComercialDAOImpl implements ComercialDAO {
	...
	public void delete(long id) {
		...
		int rows0 = jdbcTemplate.update("DELETE FROM pedido where id_comercial = ?", id);
		int rows = jdbcTemplate.update("DELETE FROM comercial WHERE id = ?", id);

		int rowFinal = rows + rows0;

		log.info("Delete de Comercial con {} registros eliminados.", rowFinal);
	}
}
